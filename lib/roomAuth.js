...
function findRoomMember(room, { uid, accessToken, email } = {}) {
  ...
  if (accessToken) {
    const member = members.find((candidate) => tokenMatches(accessToken, candidate.accessTokenHash));
    if (member) return member;
  }
  if (email) {
...
function joinRoom(room, { uid, accessToken, name, avatarColor, email }) {
  ...
  if (existing) {
    ...
    const nextToken = accessToken && tokenMatches(accessToken, existing.accessTokenHash) ? accessToken : createAccessToken();
