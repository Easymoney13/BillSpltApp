...
function assessOcrReadability(receipt, options = {}) {
  ...
  // Language ratios are guardrails against transliteration/mojibake, not a
  // requirement that every purchased row use the same script. Israeli menus
  // routinely mix English brands with Hebrew descriptions, so only a near-
  // complete loss of the expected script is a hard failure.
  if (language === 'hebrew' && hebrewCharacterRatio < 0.2) reasons.push('hebrew-script-mismatch');
  if (language === 'hebrew' && hebrewLineRatio < 0.35) reasons.push('too-few-hebrew-item-lines');
  if (language === 'mixed' && hebrewCharacterRatio < 0.08) reasons.push('mixed-script-mismatch');
  if (language === 'english' && latinCharacterRatio < 0.2) reasons.push('english-script-mismatch');
  if (Number.isFinite(recognitionConfidence) && recognitionConfidence < 25) reasons.push('low-engine-confidence');
  ...
  if (language === 'hebrew') score -= Math.max(0, 0.45 - hebrewCharacterRatio) * 0.55;
  if (language === 'mixed') score -= Math.max(0, 0.12 - hebrewCharacterRatio) * 0.4;
...
function hasRequiredHebrewVerification(receipt) {
  ...
  if (source === 'gemini-vision') {
    return ['exact-cross-model-agreement', 'review-required'].includes(status);
  }
  if (source === 'client-tesseract') {
    return ['dual-hebrew-pass-agreement', 'single-hebrew-pass-review'].includes(status);
  }
  return false;
}
