  ...
  generateUniqueRoomCode() {
    if (isTesting) {
      ...
      const occupied = new Set([
        ...Object.values(data.sessions || {}).map((s) => s.code),
        ...Object.values(data.groups || {}).map((g) => g.code),
      ]);
      return createUniqueRoomCode(occupied);
    }
    return (async () => {
      const db = getFirestore();
      let attempts = 0;
    ...
    if (isTesting) {
      ...
      (group.bills || []).forEach((bill) => {
        ...
        if (session && Array.isArray(session.members)) {
          session.members = session.members.filter((m) => m.id !== member.id && m.id !== memberId);
