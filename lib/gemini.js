...
- Set documentLanguage from the script visibly used on purchased-item rows. When Hebrew is visible, return real Hebrew Unicode in logical reading order; never transliterate it, reverse the words, or return mojibake such as repeated ×/Ã/� glyphs.
...
function receiptOcrEvidence(receipt, evidence) {
  return {
    ...
    nameVerificationStatus: requiresStrictLanguageAgreement(receipt)
      && evidence?.verificationStatus === 'cross_model_agreement'
  ...
  for (const modelName of models) {
    ...
    try {
      ...
      if (verificationBudget < 2_000) {
        if (requiresStrictLanguageAgreement(receipt)) return null;
        return {
          ...receipt,
      ...
      const verificationStatus = !verifiedReceipt
        ...
        : (sameLines
          ...
          : 'row_disagreement');
      if (requiresStrictLanguageAgreement(receipt) && !sameValues) return null;
      return {
        ...receipt,
