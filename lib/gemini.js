const { normalizeAmount, isTotalOrTaxLine, reconcileReceipt } = require('./receiptMath');
...
function normalizeReceipt(parsed, sourceText) {
  ...
  const items = rawItems.flatMap((item, index) => {
    ...
    const explicitLineTotal = normalizeAmount(
      item?.lineTotal ?? item?.line_total ?? item?.totalPrice ?? item?.total_price ?? item?.price
    );
    const calculatedLineTotal = quantity && unitPrice
      ? Math.round(quantity * unitPrice * 100) / 100
      : null;
    const price = explicitLineTotal ?? calculatedLineTotal ?? unitPrice;
...
async function requestModel(modelName, apiKey, base64Image, mimeType, verificationContext = '') {
...
${verificationContext ? `\nVerification pass: ${verificationContext}\nRe-read all item rows from the pixels and correct only genuine reading or row-alignment errors.` : ''}`;
...
function receiptQualityScore(receipt) {
  ...
  let score = receipt.items.length * 5;
...
function selectBetterReceipt(first, second) {
  ...
  return receiptQualityScore(second) > receiptQualityScore(first) ? second : first;
}
...
async function parseReceiptImage(base64Image, mimeType = 'image/jpeg', customApiKey = '') {
  ...
  for (const modelName of models) {
    try {
      ...
      const reconciliation = reconcileReceipt(receipt);
      if (!reconciliation.needsReview) return receipt;

      const verificationContext = reconciliation.receiptTotal === null
        ? `The first read found ${receipt.items.length} items but did not find a reliable printed final total.`
        : `The first read found ${receipt.items.length} items totaling ${reconciliation.itemTotal}, while the printed total was ${reconciliation.receiptTotal}.`;
      const verifiedReceipt = await requestModel(
        modelName,
        apiKey,
        cleanBase64,
        mimeType,
      ...
      return selectBetterReceipt(receipt, verifiedReceipt);
    } catch (err) {
