...
// `latest` alias moves to a new release. Prioritize Gemini 3.6 Flash / Flash Latest for speed, low cost, and accuracy.
const DEFAULT_MODELS = ['gemini-3.6-flash', 'gemini-flash-latest', 'gemini-3.7-flash', 'gemini-3.5-flash'];
...
function receiptOcrEvidence(receipt, evidence) {
  ...
  const namesAgree = ['cross_model_agreement', 'value_disagreement'].includes(evidence?.verificationStatus);
  return {
    ...evidence,
  ...
  for (const modelName of models) {
    ...
    try {
      ...
      const verifiedReceipt = await requestModel(verificationModelName, apiKey, cleanImages, mimeType, {
        pass: 'verification',
        timeoutMs: Math.min(20_000, verificationBudget),
      });
