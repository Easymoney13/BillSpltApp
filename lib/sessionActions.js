...
const HOST_ACTIONS = new Set(['SPLIT_EVERYONE', 'ADD_ITEM', 'EDIT_ITEM', 'DELETE_ITEM', 'SETTLE_ALL']);
...
function resolveActor(session, actor = {}, payload = {}) {
  ...
  const byPayloadMemberId = payload.memberId ? members.find((member) => member.id === payload.memberId) : null;
  const singleMemberFallback = members.length === 1 ? members[0] : null;
  return byUid || byMemberId || byPayloadMemberId || singleMemberFallback || null;
}
